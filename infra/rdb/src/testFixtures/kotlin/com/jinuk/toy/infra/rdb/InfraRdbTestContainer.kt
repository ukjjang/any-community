...
@Testcontainers
@Ignore
interface InfraRdbTestContainer {
    companion object {
        private val NET_WORK = Network.newNetwork()
        private const val NET_WORK_ALIASES = "toy-network"
        private const val DB_NAME = "toy-network"
        ...
        fun registerProperties(registry: DynamicPropertyRegistry) {
            registry.add("datasource.rdb.writer.jdbc-url") { DB.jdbcUrl }
            registry.add("datasource.rdb.writer.username") { DB_USERNAME }
            registry.add("datasource.rdb.writer.password") { DB_PASSWORD }

            registry.add("datasource.rdb.reader.jdbc-url") { DB.jdbcUrl }
            registry.add("datasource.rdb.reader.username") { DB_USERNAME }
            registry.add("datasource.rdb.reader.password") { DB_PASSWORD }
        }
